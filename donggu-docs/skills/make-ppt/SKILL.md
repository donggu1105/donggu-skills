---
name: make-ppt
description: Use when building an HTML presentation, slide deck, or talk slides for a lecture, seminar, workshop, pitch, or launch — or converting a PPT/PPTX to web, or enhancing an existing HTML deck. Also use when the project has a designs/ folder or a DESIGN.md to drive the look. Triggers include "PPT 만들어줘", "덱 만들어줘", "발표자료 만들어줘", "HTML 슬라이드", "presentation", "slides".
---
...
On this path the chosen `DESIGN.md` — its tokens **and** its Do's/Don'ts — **overrides the Design Aesthetics guidance above**. Render the design faithfully; never "improve" or override it. A `DESIGN.md` that says "no accent colour" or "pure black canvas" is a constraint, not a mistake.

**Format and the DESIGN.md → deck-CSS translation: see [designs-md-guide.md](designs-md-guide.md).**
...
Use the **current runtime's user-interaction mechanism** to collect all four decisions below. Prefer one batched, multi-field interaction when the runtime supports it. Otherwise ask one concise grouped question; use sequential choice prompts only when the runtime cannot represent the batch. Do not assume a harness-specific question tool.
...
**If a content `.md` exists** (e.g. `designs/<name>/<deck>.md`) or the user provides one, use it as the content source instead of asking — it already holds the title and per-slide content. See [designs-md-guide.md](designs-md-guide.md) for the format.
...
2. **View each image** — Use the **current runtime's image/vision capability**. Examples: Hermes `vision_analyze`, Claude multimodal file reading, or Codex image input. If the runtime has no image capability, report that limitation and ask the user to select usable images; do not infer visual quality from filenames.
3. **Evaluate** — For each: what it shows, USABLE or NOT USABLE (with reason), what concept it represents, dominant colors
4. **Co-design the outline** — Curated images inform slide structure alongside text. This is NOT "plan slides then add images" — design around both from the start (e.g., 3 screenshots → 3 feature slides, 1 logo → title/closing slide)
5. **Confirm with the current runtime's user-interaction mechanism**: "Does this slide outline and image selection look right?" Options: Looks good / Adjust images / Adjust outline
...
Save previews to `.presentation-previews/` (style-a.html, style-b.html, style-c.html). Each should be self-contained, ~50-100 lines, showing one animated title slide.
...
- [designs-md-guide.md](designs-md-guide.md) — **(.md-driven path only)** how to translate a `DESIGN.md` into the deck

**If the `.md`-driven path is active:** translate the chosen `DESIGN.md` into the deck's inline CSS — its colour/type/spacing tokens become `:root` variables, its components become CSS, and its Do's/Don'ts are hard constraints. Load fonts per the design's `fonts.md`. The deck still stays a single self-contained HTML file, and the viewport-fitting rules still win. See [designs-md-guide.md](designs-md-guide.md).
...
1. **Clean up** — Delete `.presentation-previews/` if it exists
...
| [designs-md-guide.md](designs-md-guide.md)         | The `.md`-driven path — designs/ library, DESIGN.md format, DESIGN.md → CSS translation | Phase 0, 2, 3 (.md-driven) |
