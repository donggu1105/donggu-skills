...
class NativePluginPackageTests(unittest.TestCase):
    ...
    def test_sns_claude_and_hermes_manifests_share_identity_and_version(self):
        ...
        self.assertEqual("2.9.0", claude["version"])
    ...
    def test_codex_marketplace_exposes_only_sns_from_existing_domain_path(self):
        ...
        self.assertEqual("2.9.0", entry["version"])
        self.assertEqual(
    ...
    def test_sns_codex_manifest_reuses_all_skills_and_matches_release_versions(self):
        ...
        self.assertEqual("2.9.0", codex["version"])
    ...
    def test_sns_registers_exact_native_tool_surface(self):
        ...
        self.assertIn("PUBLISHER_API_TOKEN", dispatch["requires_env"])
        self.assertIn("SUPABASE_SERVICE_KEY", dispatch["requires_env"])

    def test_sns_requirement_check_requires_local_publisher_token(self):
        module_name = "donggu_sns_requirement_contract_test"
        ...
        with mock.patch.dict("os.environ", complete, clear=True):
            self.assertTrue(tools.check_requirements())
        without_publisher = {
            key: value for key, value in complete.items()
            if key != "PUBLISHER_API_TOKEN"
        }
        with mock.patch.dict("os.environ", without_publisher, clear=True):
            self.assertFalse(tools.check_requirements())
