...
```
Personal Branding/50_Channel_Packs/1_SNS/YouTube/_references/SOURCE - <라벨>.md
```

- **NOT `10_Sources/`** — that's the high-level curated box; dumping every video there clutters it. External video captures belong in the YouTube channel pack's reference layer (the `REFERENCE - YouTube` swipe concept).
...
| Mistake | Fix |
|---|---|
| Saved to `10_Sources/` | External captures → `1_SNS/YouTube/_references/` |
...
## Real-world check
잡소리 ep.43 (원티드 정기수 AX, 27:32) → `1_SNS/YouTube/_references/SOURCE - 잡소리 ep43 원티드 정기수 AX.md`, summary + ~25 jump-links. Verified 2026-06-12.
