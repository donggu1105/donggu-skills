...
허용 root는 `10_Sources`, `20_Core`, `40_Channel_Packs`, `50_MOCs`다. 허용 root 4종은 의도된 제품 범위다 — `30_Foundations`·`60_Projects`·`80_Cases`의 부재는 누락이 아니라 결정이며, 확장은 별도 보안 리뷰를 거친다. `40_Snippets`(폐기 폴더), `00_Inbox`, 절대경로, `..`, symlink, 비정규 파일, binary/non-UTF-8, source/MOC hash mismatch, existing new CORE는 거부한다. `recommend_only`, merge/classify/status cleanup과 action 외 변경은 release 후 새 후보가 필요하다.
