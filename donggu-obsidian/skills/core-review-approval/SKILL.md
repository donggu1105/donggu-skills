...
8. action을 한 번만 적용한다. 실행 가능한 결정적 계약은 다음뿐이다.
   - `replace`: source에서 `old`가 정확히 한 번 존재할 때만 `new`로 교체
   - `remove_link`: `old`가 정확히 한 번 존재할 때만 제거
9. `add_link`는 현재 action에 insertion anchor가 없으므로 위치를 추측하지 않는다. `create_core_with_backlink`도 완성 target/body/backlink가 action에 없으면 실행하지 않는다. 이 둘과 `recommend_only`, `merge`, `classify`, `status_cleanup`, `skill_drift`, 자연어 설명뿐인 action은 `release_core_review_candidate(...,'action requires deterministic re-evaluation')` 후 구체적인 새 후보를 요청한다.
