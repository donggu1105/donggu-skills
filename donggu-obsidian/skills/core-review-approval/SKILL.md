...
2. `donggu_core_recovery_status(vault_root)`를 read-only로 호출한다. 오직 exact `state=no_transaction`, `candidate_code=null`, `transaction_sha256=null`일 때만 계속한다. `prepared|rolled_back|committed`, malformed result, exit 4/5/6이면 현재 후보를 plan/prepare/send하지 않고 기존 transaction을 먼저 reconcile한다.
...
- **exit 4**: clean pre-mutation이 증명될 때만 위 release + revoke를 수행한다. malformed journal, rollback incomplete, recovery failure면 release/revoke하지 않고 운영자 복구로 보낸다.
...
- **ack/confirm 응답 유실**: receipt status와 recovery status를 먼저 읽는다. journal이 committed면 same nonce ack만 retry하고, clean이면 DB confirm만 retry한다. 결과를 증명하지 못하면 ambiguous로 유지한다.
