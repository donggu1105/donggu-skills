...
description: Use when a blog/SNS post needs an AI-generated image (대표이미지·삽화)
  ...
  Pollinations(무료 API), Cloudflare(무료티어), Gemini(nano-banana 직결·무료티어), fal.ai(유료 nano-banana). 모델 선택형
...
| `gemini` | **무료 티어**(하루 제한)/유료 ~$0.04/장 | `GEMINI_API_KEY` | nano-banana(gemini-2.5-flash-image) 직결. 품질 상, 종횡비 자동 매핑 |
| `fal` | **유료**(~$0.05~0.15/장) | `FAL_KEY` | nano-banana 등 최고품질 필요 시 (Gemini 키 있으면 gemini가 더 쌈) |
