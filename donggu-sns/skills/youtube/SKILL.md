...
실제 Shorts 영상 아티팩트 제작은 `make-shorts`, 주간 인박스 CORE 추출은 `extract-core`, 실제 SNS 게시는 `publish-sns`를 사용한다.
...
Personal Branding/50_Channel_Packs/1_SNS/YouTube/INDEX - YouTube.md
Personal Branding/50_Channel_Packs/1_SNS/YouTube/_anchors/
```
...
최종 확정이 아니라 제작 가치 검증용 가설이다. 편집 후 영상이 약속을 실제 이행하는지 다시 확인한다.

### 4. Build Episode

`Videos/<YYYY-MM-slug>/`에 다음 파일을 만든다.
...
- `01-preproduction`: 수요·출처·경험/추정 분리·반론·아웃라인·샷·합성 데이터
