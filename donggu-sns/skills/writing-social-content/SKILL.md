...
- 두 맥락 중 정확히 하나만 source 또는 요청/브리프에 명시되면, 고객·조직 맥락을 source 또는 요청/브리프가 명시적으로 제공하면 `FDE`, 자기 제품·시장 가설 맥락을 source 또는 요청/브리프가 명시적으로 제공하면 `1인 빌더`를 사용하고 다시 묻지 않는다.
- 두 맥락이 모두 나타나면 사용자가 명시한 페르소나가 우선한다. 명시한 페르소나가 없으면 중심 주장을 소유하는 주 관점만 본다. source 또는 요청/브리프가 고객·조직 변화가 중심 주장임을 분명히 하면 `FDE`, source 또는 요청/브리프가 자기 제품·시장 검증이 중심 주장임을 분명히 하면 `1인 빌더`를 선택한다. 오디언스가 명시되지 않아도 주 페르소나를 정확히 하나 선택한다. 오디언스는 별도로 정하며 페르소나를 결정하지 않는다. 주 관점이 불명확하고 선택이 논지를 바꾸면 `FDE와 1인 빌더 중 어떤 페르소나로 쓸까요?`라고 묻는다.
- 두 페르소나를 동시에 선택하거나 `혼합`을 만들지 않는다.
- 직접 제작·자동화·AX·지속 사용만으로는 페르소나를 추론하지 않는다. 두 맥락이 모두 없으면 설명이나 추가 선택지 없이 `FDE와 1인 빌더 중 어떤 페르소나로 쓸까요?` 한 문장만 묻는다.
...
- 두 페르소나가 만나는 소재도 중심 주장을 소유하는 주 관점에서 주 페르소나 하나를 먼저 고른다.
...
8. 주 페르소나가 정확히 하나이며 논지·근거가 선택한 페르소나의 관점과 일치하는가.
9. 오디언스는 글마다 별도로 검수하며 페르소나를 추론하거나 제한하는 데 사용하지 않았는가.
10. `DA`나 `AX Engineer`를 별도 공개 페르소나처럼 다루지 않았는가.
11. FDE 글의 고객·회사 맥락과 1인 빌더 글의 출시·사용자·수익 주장이 source 근거 안에 있는가.
...
| 두 페르소나를 주 페르소나로 혼합하기 | 두 경험을 잇더라도 중심 주장을 소유하는 주 관점에서 주 페르소나 하나를 먼저 고른다. |
