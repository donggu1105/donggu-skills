# 발신 페르소나

공개 페르소나 선택지는 정확히 두 개다. 모든 지원 채널은 두 페르소나 모두 사용할 수 있고, 오디언스는 글마다 별도로 정한다. 페르소나는 보이스가 아니라 어떤 문제와 증거를 중심에 놓을지 정하는 관점이다.

## 선택 규칙

1. 한 초안에는 주 페르소나를 정확히 하나만 둔다.
2. 사용자가 페르소나를 명시했으면 다시 묻지 않는다.
3. 두 맥락 중 정확히 하나만 source 또는 요청/브리프에 명시되면, 고객·조직 맥락을 source 또는 요청/브리프가 명시적으로 제공하면 `FDE`, 자기 제품·시장 가설 맥락을 source 또는 요청/브리프가 명시적으로 제공하면 `1인 빌더`를 사용하고 다시 묻지 않는다.
4. 두 맥락이 모두 나타나면 사용자가 명시한 페르소나가 우선한다. 명시한 페르소나가 없으면 중심 주장을 소유하는 주 관점만 본다. source 또는 요청/브리프가 고객·조직 변화가 중심 주장임을 분명히 하면 `FDE`, source 또는 요청/브리프가 자기 제품·시장 검증이 중심 주장임을 분명히 하면 `1인 빌더`를 선택한다. 오디언스가 명시되지 않아도 주 페르소나를 정확히 하나 선택한다. 오디언스는 별도로 정하며 페르소나를 결정하지 않는다. 주 관점이 불명확하고 선택이 논지를 바꾸면 `FDE와 1인 빌더 중 어떤 페르소나로 쓸까요?`라고 묻는다.
5. 두 페르소나를 동시에 선택하거나 `혼합`을 만들지 않는다.
6. 선택에 따라 논지가 달라지는데도 모호하면 `FDE`와 `1인 빌더` 중 하나를 한 번만 묻는다.
7. 두 경험을 잇는 글도 중심 주장을 소유하는 주 관점에서 주 페르소나 하나를 먼저 고른다.
8. `직접 만들었다`, `자동화`, `AX` 같은 공통 신호만으로는 페르소나를 추론하지 않는다. `직접 만든 자동화가 일주일 뒤에도 계속 쓰였다`처럼 지속 사용만으로는 고객 채택이나 제품 검증 맥락을 추정하지 않는다. 명시된 고객·조직 맥락과 자기 제품·시장 가설 맥락이 둘 다 없으면 `FDE`와 `1인 빌더` 중 하나를 한 번만 묻는다.
9. 질문할 때는 설명이나 추가 선택지 없이 다음 한 문장만 사용한다: `FDE와 1인 빌더 중 어떤 페르소나로 쓸까요?`

## FDE

- 표기: `Forward Deployed Engineer`는 canonical `FDE`의 확장 표기이자 별칭이다.
- 중심 질문: 고객·조직의 어떤 문제를 어떤 성과와 지속 가능한 업무 변화로 바꿀 것인가.
- 다루는 근거: 문제 발견, 인터뷰, 설계, 구현, 검증, 채택, 운영 전환, 인수인계, AX 현장 관찰.
- 관점: 기술 도입보다 문제에서 성과까지의 책임 범위와 내가 떠난 뒤에도 남는 변화를 본다.
- 오디언스 후보: AX 의사결정자, 현장 실무자, 개발자, FDE·AI 엔지니어. 오디언스는 글마다 고르며 채널에 고정하지 않는다.
- 경계: 사용자나 source가 제공하지 않은 고객 장면, 성과, 고객·회사 식별정보, 내부 의사결정은 만들지 않는다.

## 1인 빌더

- 중심 질문: 내 가설을 무엇으로 만들고 어떻게 시장과 사용자에게 검증할 것인가.
- 다루는 근거: 문제 선택, 제품 결정, 직접 구현, 출시, 사용자 반응, 운영, 자동화, 수익 모델, 실패와 반복.
- 관점: 혼자 만든다는 사실보다 가설을 제품과 시장 증거로 바꾸는 전 과정을 본다.
- 오디언스 후보: 1인 창업가, 개발자 빌더, 비개발 프로덕트 메이커. 오디언스는 글마다 고르며 채널에 고정하지 않는다.
- 경계: 실제 근거가 있을 때만 출시, 사용자, 매출·수익, 사업 성과를 말한다. `솔로프리뉴어`, `1인 개발자`, `1인 창업가`는 문맥상 별칭이지 별도 페르소나가 아니다.

## 공통 기반과 제외

- AI Product Engineering, AX, 현장 우선, 직접 제작, 검증 우선은 두 페르소나의 공통 기반이다.
- AX Engineer는 공통 전문 영역을 설명하는 표현이며 세 번째 페르소나가 아니다.
- DA는 위시켓 내부 역할이며 이 스킬의 공개 페르소나 선택지에 넣지 않는다.
- `AX Engineer`와 `DA`는 `FDE`의 별칭이 아니며 공개 페르소나 선택지도 아니다.
- 페르소나 선택은 source의 사실 권한을 넓히지 않는다.
